...
// Komponente zur Darstellung eines Zeitstrahls mit Beobachtungsdaten ausgewählter Vogelarten
export default function VogelZeitstrahl({ birdIds, range, setRange, startDate, endDate }) {
  const [data, setData] = useState([]);

  // Temporärer Bereich für den Slider, der vor dem finalen Setzen des Zeitbereichs genutzt wird
  const [tempRange, setTempRange] = useState([range[0].getTime(), range[1].getTime()]);

  // Wird bei jeder Bewegung des Sliders ausgelöst (noch nicht bestätigt)
  const handleChange = (event, newValue) => {
    setTempRange(newValue);
  };

  // Wird ausgelöst, wenn der Benutzer das Verschieben des Sliders abgeschlossen hat
  const handleChangeCommitted = (event, newValue) => {
    const newRange = newValue.map((ts) => new Date(ts));
    setRange(newRange);
  };

  // Holt Beobachtungsdaten für die ausgewählten Vogelarten bei Änderungen
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then((data) => {
        // Formatieren und sortieren der Daten nach Zeitstempel
